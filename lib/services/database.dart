...
class DatabaseService {
  ...
  final CollectionReference restaurantReviewCollection =
      Firestore.instance.collection("restaurantReviews");
  final CollectionReference userData =
      Firestore.instance.collection("userData");
  final CollectionReference userCheckIns =
      Firestore.instance.collection("userCheckIns");

  

  
  Future<void> updateUserData(
      String email, String name, String picturePath) async {
    return await userData.document(uid).setData(
        {'id': uid, 'email': email, 'name': name, 'picturePath': picturePath});
  }
  ...
  Future updateRestaurantRating(Restaurant restaurant, double rating) async {
    DocumentReference _restaurantReference =
        restaurantCollection.document(restaurant.id);
    await _restaurantReference.updateData({'rating': rating});
  }

  Future deleteCheckIn(CheckIn checkin) async {
    await userCheckIns.document(checkin.id).delete();
  }

  Future<Restaurant> getRestaurantFromId(String id) async {
    return restaurantCollection.document(id).get().then((doc) => Restaurant(
        city: doc.data['city'] ?? '',
  ...
  Future<List<RestaurantReview>> getARestaurantsReviews(
      Restaurant restaurant) async {
    return restaurantReviewCollection
        .where('restaurant', isEqualTo: restaurant.id)
        .getDocuments()
        .then((querysnapshot) async {
      List<RestaurantReview> reviews =
          _restaurantReviewListFromSnapshot(querysnapshot, restaurant);
      for (RestaurantReview review in reviews) {
        await getUserFromId(review.foodie.uid)
            .then((value) => review.foodie = value);
      }
      return reviews;
    });
  }

  List<RestaurantReview> _restaurantReviewListFromSnapshot(
      QuerySnapshot snapshot, Restaurant res) {
    return snapshot.documents.map((doc) {
      return RestaurantReview(
        ...
        photos: doc.data['photos'].cast<String>() ?? new List<String>(),
        photoDescriptions:
            doc.data['photoDescriptions'].cast<String>() ?? new List<String>(),
  ...
  Future addRestaurantReview(RestaurantReview resreview) async {
    return await restaurantReviewCollection.document().setData({
      ...
      'photoDescriptions': resreview.photoDescriptions,
  ...
  Future<List<RestaurantReview>> getCitysReviews(String city) async {
    return restaurantReviewCollection
        ...
        .then((querysnapshot) async {
      List<RestaurantReview> reviews =
          _restaurantReviewListFromSnapshot(querysnapshot, null);
      for (RestaurantReview review in reviews) {
        User haps = await getUserFromId(review.foodie.uid);
        await getRestaurantFromId(review.restaurant.id)
            .then((value) => review.restaurant = value);
        review.foodie = haps;
      }
      return reviews;
    });
...
// gets all checkins from one user
  Future<List<CheckIn>> getUserCheckIn() async {
    return userCheckIns
      ...
      for (CheckIn checkIn in checkins) {
        ...
        checkIn.foodie = user;
        await getRestaurantFromId(checkIn.restaurant.id)
            .then((value) => checkIn.restaurant = value);
      }
  ...
  Future<Restaurant> getRestaurantFromCheckinCode(String checkincode) {
    return restaurantCollection
        .where('checkincode', isEqualTo: checkincode)
        .getDocuments()
        .then((value) => _resturantListFromSnapshot(value)[0]);
  }
